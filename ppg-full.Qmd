...
In particular, DNA analysis revealed that ferns are sister to seed plants (thus "pteridophytes" are not a natural group) and that horsetails and whisk ferns are not merely "fern allies" but nested within ferns [@Pryer2001].
The first major revised classification of the molecular-phylogenetic era was @Smith2006a, but this only treated taxa at the level of family and higher.
The next major taxonomy was Pteridophyte Phylogeny Group (PPG) I [-@PteridophytePhylogenyGroupI2016], which extended the taxonomic scope to genera.
...
Another change is the recognition of XX nothogenera, which were not treated at all in PPG I [-@PteridophytePhylogenyGroupI2016].
...
Recent phylogenies show that the myrmecophilous *Lecanopteris* s.s. is nested within a clade that includes taxa formerly placed in *Microsorum*, but that this clade is distinct from the lineage containing the type species of *Microsorum* [@Chen2020; @Perrie2021].
...
ppg_tl |>
  ...
  as_tibble() |>
  janitor::clean_names() |>
  mutate(indent = glue::glue("{indent}* ") |> as.character()) |>
  select(
